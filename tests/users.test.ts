...
test("GET /users/:userId hides private fields and inactive services from other users, and returns 404 for non-discoverable users", async () => {
  const { testApp, viewer } = await createUsersTestContext();
  ...
  try {
    ...
    assert.deepEqual(hiddenResponse.json(), { error: "user_not_found" });
